  ...
  return (
    <div className="flex flex-col h-full bg-[#0A111E] overflow-hidden">
      ...
      <div className="grid grid-cols-4 gap-3 px-6 py-4 shrink-0">
        {[
          ...
          <div key={s.label} className="bg-[#122036] border border-white/[0.06] rounded-[16px] px-5 py-4 flex items-center gap-4">
            ...
            <div>
              <div className="text-xl font-normal text-[#F4F6F9] leading-none">{s.value}</div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6 grid grid-cols-[1fr_360px] gap-4">
        {/* Pedidos */}
        <div className="bg-[#122036] border border-white/[0.06] rounded-[16px] overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
            <h2 className="text-base font-semibold text-[#E9EEF4]">Pedidos de compra</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {pedidos.length === 0 ? (
              <p className="text-center py-12 text-[#4F6178] text-sm">Nenhum pedido</p>
            ) : pedidos.map(p => {
              const s = STATUS_PEDIDO[p.status ?? ''] ?? { label: p.status ?? '—', color: '#5C6E84', bg: 'rgba(92,110,132,0.12)' }
              return (
                <div key={p.id} className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.03] cursor-pointer last:border-0">
                  <div>
                    <div className="text-sm font-medium text-[#E9EEF4]">{p.descricao ?? `Pedido #${p.id}`}</div>
                    <div className="text-[11px] text-[#5C6E84] mt-0.5">{p.fornecedores?.nome ?? '—'} · {fmtData(p.data_pedido ?? p.created_at)}</div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-sm font-semibold text-[#E9EEF4]">{fmtBRL(p.valor_total)}</span>
        ...
        <div className="bg-[#122036] border border-white/[0.06] rounded-[16px] overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
            <h2 className="text-base font-semibold text-[#E9EEF4]">Fornecedores</h2>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.06] hover:bg-white/[0.1] text-[#D4DEEA] text-xs font-semibold rounded-[8px] transition-colors">
              <Plus size={13} /> Novo
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            {fornecedores.length === 0 ? (
              <p className="text-center py-12 text-[#4F6178] text-sm">Nenhum fornecedor</p>
            ) : fornecedores.map(f => {
              ...
              return (
                <div key={f.id} className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.03] cursor-pointer last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-[#E9EEF4] truncate">{f.nome}</div>
                    <div className="text-[11px] text-[#5C6E84] truncate">{f.contato ?? '—'}{f.telefone ? ` · ${f.telefone}` : ''}</div>
                  </div>
                  <span className="text-[11px] text-[#5C6E84] shrink-0">{qtd} pedidos</span>
                </div>
              )
            })}
          </div>
