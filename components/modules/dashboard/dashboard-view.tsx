...
import { formatCurrency, formatRelativeTime, LEAD_STATUS_COLORS, CANAIS_VENDA } from '@/lib/utils'
...
export function DashboardView({ data }: { data: DashboardData }) {
  const { kpis, vendasRecentes, leadsRecentes } = data
    ...
    <main className="flex-1 overflow-y-auto scrollbar-thin px-[30px] py-7">
      <div className="max-w-[1320px] mx-auto space-y-5">
        ...
        <section className="relative overflow-hidden rounded-[24px] p-[36px_40px] bg-hero-gradient border border-white/[0.08] shadow-[0_28px_70px_rgba(0,0,0,0.45)] animate-fade-up">
          ...
          <div className="relative z-10 max-w-[620px]">
            ...
            <div className="flex items-end gap-[18px] mt-[26px]">
              <div>
                <div className="font-mono text-[10px] tracking-[0.18em] text-[#7E8EA2]">FATURAMENTO · ESTE MÊS</div>
                <div className="font-serif font-medium text-[52px] leading-none tracking-[-0.03em] text-white mt-1.5">
        ...
        <div className="grid grid-cols-4 gap-[18px]">
          <KpiCard label="VENDAS TOTAIS" value={formatCurrency(kpis.receitaMes)} sub="este mês" delta="+18,2%" deltaUp delay={50} />
          <KpiCard label="LUCRO BRUTO" value={formatCurrency(kpis.lucroMes)} sub={`margem de ${margem}%`} delta="+12,4%" deltaUp delay={120} />
          <KpiCard label="TICKET MÉDIO" value={formatCurrency(kpis.ticketMedio)} sub="por venda fechada" delta="+5,1%" deltaUp delay={190} />
          <KpiCard label="VENDAS FECHADAS" value={String(kpis.qtdVendasMes)} sub="no período" delta="+8,3%" deltaUp delay={260} />
        </div>
