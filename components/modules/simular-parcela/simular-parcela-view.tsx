  ...
  return (
    <main className="flex-1 overflow-y-auto scrollbar-thin px-[30px] py-7">
      <div className="max-w-[920px] mx-auto animate-fade-up">
        ...
        <div className="bg-[#122036] border border-white/[0.06] rounded-[20px] p-[24px_26px] mb-[18px] flex items-center gap-6 flex-wrap">
          <div className="flex-1 min-w-[220px]">
            <div className="font-mono text-[10px] tracking-[0.16em] text-[#6B7C92] mb-[6px]">VALOR DA VENDA</div>
            <div className="flex items-center gap-2">
              <span className="font-serif text-[26px] text-[#6B7C92]">R$</span>
              <input
                type="text"
                inputMode="decimal"
                value={valor}
                ...
                placeholder="0,00"
                className="flex-1 bg-transparent border-none border-b-2 border-[rgba(215,40,47,0.4)] text-[#F4F6F9] font-serif text-[34px] outline-none pb-[2px] min-w-0 focus:border-[#F0656B] transition-colors placeholder:text-[#3A4A63]"
              />
            </div>
          </div>
          <p className="text-[12.5px] text-[#8A9BB0] max-w-[260px] leading-[1.5]">
            Compare <strong className="text-[#E9EEF4]">crédito</strong> e{' '}
            <strong className="text-[#E9EEF4]">link de pagamento</strong> — cada um com suas taxas.
              ...
              <button
                ...
                className={cn(
                  ...
                  bandeira === b.id
                    ...
                    : 'bg-transparent border-white/[0.06] text-[#6B7C92] hover:border-white/[0.12] hover:text-[#9FB0C2]'
                )}
              >
                {b.label}
              </button>
        ...
        {/* ── Tabela ── */}
        <div className="bg-[#122036] border border-white/[0.06] rounded-[20px] overflow-hidden">
          {/* Header */}
          ...
          {loading ? (
            <div className="text-center py-12 text-[#5C6E84] font-mono text-[12px]">Carregando taxas...</div>
          ) : rows.length === 0 ? (
            <div className="text-center py-12 text-[#5C6E84] font-mono text-[12px]">Sem taxas cadastradas para esta modalidade.</div>
          ) : (
            rows.map((r, i) => (
              <div
                key={r.parcelas}
                className={cn(
                  'grid gap-3 px-6 py-[13px] border-b border-white/[0.04] last:border-0 transition-colors hover:bg-white/[0.025]',
                  r.parcelas === 1 && 'bg-white/[0.015]'
                )}
                style={{ gridTemplateColumns: '1fr 1fr 1.3fr 1.3fr' }}
              >
                {/* Parcelas */}
                <div className="text-[14px] font-bold text-[#F4F6F9]">
                  {r.parcelas}x
                  {r.parcelas === 1 && (
                ...
                {/* Taxa */}
                <div className="text-right font-mono text-[12.5px] text-[#8A9BB0]">
                  {r.taxa > 0 ? `${r.taxa.toFixed(2)}%` : '—'}
                </div>

                {/* Valor da parcela */}
                <div className="text-right text-[13.5px] font-semibold text-[#E9EEF4]">
